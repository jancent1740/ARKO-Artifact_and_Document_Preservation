...
import { ChevronDown, ChevronRight, LogOut, Users, Plus, FolderPlus, LayoutList } from 'lucide-react';
...
export function CuratorSidebar({
  ...
  useEffect(() => {
    if (['manage-items', 'digital-archives-all-items', 'digital-archives-projects', 'digital-archives-needs-attention', 'digital-archives-activity-history', 'create-item-draft', 'create-collection'].includes(activePage || '')) {
  ...
  return (
    <>
        ...
        <div className="px-4 py-[17px] border-b border-[#364153] flex-shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <h1 className="text-white text-[17px] leading-[28px]">ARKO Curator</h1>
            </div>
      ...
      <nav className="flex-1 px-4 py-4 space-y-1 overflow-y-auto">
        <div>
          <button
            ...
            className={`w-full flex items-center justify-between gap-4 px-4 py-2.5 rounded-lg transition-colors ${
              digitalArchivesOpen || ['manage-items', 'digital-archives-all-items', 'digital-archives-projects', 'digital-archives-needs-attention', 'digital-archives-activity-history', 'create-item-draft', 'create-collection'].includes(activePage || '')
          ...
          {digitalArchivesOpen && (
            <div className="relative ml-6 mt-1 space-y-1">
              ...
              <button
                onClick={() => handleNav('manage-items')}
                className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg transition-colors relative ${
                  activePage === 'manage-items' ? 'text-[#f3f4f6]' : 'text-[#d1d5dc] hover:bg-[rgba(106,114,130,0.15)]'
                }`}
              >
                <div className="absolute left-[-8px] top-0 h-5 w-px bg-[rgba(106,114,130,0.3)]" />
                <div className="absolute left-[-8px] top-1/2 w-2 h-px bg-[rgba(106,114,130,0.3)]" />
                <div className="absolute left-[-8px] top-1/2 bottom-0 w-px bg-[rgba(106,114,130,0.3)]" />
                <LayoutList className="w-4 h-4 flex-shrink-0 ml-2" strokeWidth={1.33} />
                <span className="text-[13px] font-semibold">Manage Items</span>
              </button>
              ...
              <button
                onClick={() => handleNav('create-collection')}
                className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg transition-colors relative ${
                  activePage === 'create-collection' ? 'text-[#f3f4f6]' : 'text-[#d1d5dc] hover:bg-[rgba(106,114,130,0.15)]'
                }`}
              >
                <div className="absolute left-[-8px] top-0 h-5 w-px bg-[rgba(106,114,130,0.3)]" />
                <div className="absolute left-[-8px] top-1/2 w-2 h-px bg-[rgba(106,114,130,0.3)]" />
                <div className="absolute left-[-8px] top-1/2 bottom-0 w-px bg-[rgba(106,114,130,0.3)]" />
                <FolderPlus className="w-4 h-4 flex-shrink-0 ml-2" strokeWidth={1.33} />
                <span className="text-[13px] font-semibold">Create Collection</span>
              </button>
              ...
              >
                ...
                <span className="text-[13px] font-semibold">Activity History</span>
              </button>
            </div>
          )}
        </div>
        ...
        >
          <Users className="w-4 h-4 flex-shrink-0" strokeWidth={1.33} />
          <span className="text-[13px] font-semibold">Staff Management</span>
        </button>
      </nav>
      ...
      <div className="flex-shrink-0 px-4 py-4 border-t border-[#364153] bg-[#1a202c]">
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            onClick={() => { if (onLogout) onLogout(); navigate('/register'); }}
